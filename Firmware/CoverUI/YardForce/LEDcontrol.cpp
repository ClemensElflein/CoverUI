...
LEDcontrol::LEDcontrol(const uint32_t *t_kPinByLedNumPtr, const size_t t_kNumLeds, void (LEDcontrol::*t_set_base10_leds_cb)(char digit))
    : kPinByLedNumPtr(t_kPinByLedNumPtr), kNumLeds(t_kNumLeds), set_base10_leds_cb(t_set_base10_leds_cb) {
    // Set GPIO modes
...
void LEDcontrol::toggle(uint8_t led_num, bool change_state) {
    has(led_num, LED_state::LED_off) ? set(led_num, LED_state::LED_on, change_state) : set(led_num, LED_state::LED_off, change_state);
}
...
void LEDcontrol::force_(uint8_t led_num, bool force, uint32_t *force_type_) {
    ...
    force ? *force_type_ |= led_bin : *force_type_ &= ~led_bin;
}

void LEDcontrol::force_off(uint8_t led_num, bool force) {
    force_(led_num, force, &force_led_off_);
    force ? set(led_num, LED_state::LED_off, false)  // Directly set without changing state
...
void LEDcontrol::force_on(uint8_t led_num, bool force) {
    ...
    force ? set(led_num, LED_state::LED_on, false)  // Directly set without changing state
          : set(led_num, get(led_num), false);      // Restore state
}
...
void LEDcontrol::blink_timer_elapsed(LED_state blink_state) {
    ...
    // Synchronous toggle
    sync_blink_map[blink_state] == LED_state::LED_on ? sync_blink_map[blink_state] = LED_state::LED_off
