/**
 * @file LEDcontrol_RM-ECOW-V110.cpp
 ...
 *
 */
#include "LEDcontrol_RM-ECOW-V110.hpp"
...
void LEDcontrolRMECOWV110::sequence_animate_handler() {
...
void LEDcontrolRMECOWV110::show_countdown_state(unsigned int sec, LED_state state) {
 ...
 */
void LEDcontrolRMECOWV110::sequence_countdown_handler() {
 ...
 */
unsigned int LEDcontrolRMECOWV110::boot_animation()  // A short boot animation which return the amount of ms it will take
{
 ...
 */
void LEDcontrolRMECOWV110::set_base10_leds(char digit) {
...
void LEDcontrolRMECOWV110::force_off_num_seq_leds(bool force) {
    force_off(LED_NUM_LIFTED, force);        // Num change signalling LED
    for (unsigned int i = 12; i <= 17; i++)  // Base10 related LEDs
        force_off(i, force);
}

LEDcontrolRMECOWV110 leds(kPinByLedNum, sizeof(kPinByLedNum) / sizeof(uint32_t), &LEDcontrol::set_base10_leds);  // Main LED controller object
