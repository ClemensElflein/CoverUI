...
Either by self "compile and upload" via PlatformIO, or by a simple Upload via the ST-Link tool.
...
- In bottombar click `Switch PlatformIO Project Environment` and choose (whatever programmer/debugger- probe use) either: `env:yfc500_stlink` or `env:yfc500_picoprobe` 
- Wait till tools got loaded (bottom right status info)
